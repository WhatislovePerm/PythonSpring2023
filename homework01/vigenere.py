    ...
    for i in range(lengthOfPlainText):
        if plaintext[i].isalpha():
            if plaintext[i].islower():
                textCodes[i] = (textCodes[i] - firstLowerCode + keyCodes[i % lengthOfKeywordText]) % alphabetLength + firstLowerCode
            else:
                textCodes[i] = (textCodes[i] - firstUpperCode + keyCodes[i % lengthOfKeywordText]) % alphabetLength + firstUpperCode
